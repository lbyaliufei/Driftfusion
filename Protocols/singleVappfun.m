function sol_Vapp = singleVappfun(sol_ini, t_period, N_period) % Creates a single carrier device and then applies a 20 mV periodic
...
coeff = [0.5, (2*pi)/t_period,0,1];
%Vapp_func = @(coeff, t) coeff(1)*sin(coeff(2)*t + coeff(3));
Vapp_func = @(coeff, t) (coeff(4)+coeff(1)*sawtooth((t*coeff(2)+0.5*pi),0.5));

% Vapp_function(sol_ini, Vapp_func, tmax, tpoints, logtime)
sol_Vapp = Vapp_function(sol_ini, Vapp_func, coeff, tmax, 200, 0);
